...
class WalletStore {
  ...
  @action TransferETH = async(selectedWallet,selectedToken,recipientaddress,setamount,cb,cberror) =>{
    ...
    if(selectedToken.TokenType == "eth"){
      const web3 = new Web3(settingStore.selectedETHNetwork.infuraendpoint + this.infuraprojectid);
      ...
      const web3 = new Web3(settingStore.selectedETHNetwork.infuraendpoint + this.infuraprojectid);
      var CurrentNetworkAllTokenInfo = toJS(this.allTokenAsset).find(x => x.AssetCode == selectedToken.AssetCode);
      var TokenInfo = CurrentNetworkAllTokenInfo.TokenInfoList[0]; //.find(x => x.Network == settingStore.selectedWANNetwork.shortcode);
  ...
  FormatTransactionByAddress(tokentype,item){
    // console.log("FormatTransactionByAddress", JSON.stringify(item));
    var trx = {
      trxid : item.hash,
      from : item.from,
      to : item.to,
  ...
  @action LoadTransactionByAddress(tokentype,publicaddress,cb,cberror){
    ...
    if(tokentype == "erc20" || tokentype == "eth"){
      ...
      iWanUtils.getTransByAddress("WAN",publicaddress).then(response => {
        // console.log("getTransByAddress",tokentype, response)
  ...
  @action approveMultiSigTransaction(acctoken, trxid, selectedWallet,selectedToken,recipientaddress,setamount,execute,cb,cberror){
    ...
    if(execute){
      this.TransferETH(selectedWallet,selectedToken,recipientaddress,setamount,(response)=>{
        this.updateCompletedMultiSignTrx(acctoken, trxid, selectedToken, false, cb, cberror);
      },cberror);
    }else{
      this.updateCompletedMultiSignTrx(acctoken, trxid , selectedToken, true, cb, cberror);
    }
  }

  @action updateCompletedMultiSignTrx(acctoken, trxid, selectedToken, isredirect, cb, cberror){
    var network = settingStore.selectedETHNetwork.shortcode;
    if(selectedToken.TokenType == "wan" || selectedToken.TokenType == "wrc20"){
      network = settingStore.selectedWANNetwork.shortcode;
    }
  ...
  @action GetAllTokenAssetByNetwork(acctoken, cb, cberror){
    // console.log(settingStore.selectedWANNetwork.shortcode);
    var formdata = new FormData();
    formdata.append('token', acctoken);
    // formdata.append('network', settingStore.oldnetwork.shortcode);
    formdata.append('ethnetwork', settingStore.selectedETHNetwork.shortcode);
    ...
    return new Promise((resolve,reject) =>{
      ...
      selectedwallet.tokenassetlist.map(async(tokenitem,index) =>{
        if(tokenitem.TokenType == "eth"){
          // console.log(settingStore.selectedETHNetwork.infuraendpoint);
          var web3 = new Web3(settingStore.selectedETHNetwork.infuraendpoint + this.infuraprojectid);
          ...
          // console.log("settingStore.selectedWANNetwork.shortcode", settingStore.selectedWANNetwork.shortcode);
          // console.log("TokenInfo.ContractAddress", TokenInfo.ContractAddress);
          // // console.log("this.primaryTokenAsset", toJS(this.primaryTokenAsset));
          // "0x97626da35b3290c9c86df26656ede4f56c60d52b"
          //0xcb7efdb7c0cfc5fc9b3fa648abb9e0bd638dc516
          iWanUtils.getWrc20Balance("WAN",tokenitem.PublicAddress,TokenInfo.ContractAddress).then(res => {
            if (res && Object.keys(res).length) {
              var balance = res;
              // console.log("wrc20", balance);
