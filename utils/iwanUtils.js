...
const { wannetwork } = require('../libs/network');
...
class iWanUtils{//extends WsInstance {
  constructor(apiKey, secretKey, option = {}) {
    //GET NETWORK SETTING
    this._getWanNetwork(apiKey, secretKey, option);
    this.index = 0;
  }
          ...
          start: () => {
              ...
              that.intervalObj = setInterval(function ping() {
                  if (!that.wss.isAlive) {
                      --that.wss.tries;
                      if (that.wss.tries < 0) {
                          console.warn("Server [%s] is unreachable", that.ws_url);
