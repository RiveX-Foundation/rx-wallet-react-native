...
import { Color, Config, SensitiveInfo, numberWithCommas } from '../extension/AppInit';
...
@inject('settingStore')
@observer
class Home_backupbeforeerc extends Component {
  constructor(props){
    super(props);
    this.state = {
      mywalletlist:[],
      ...
      // showblur:false,
      // showsticky:false,
      selectedWallet:this.props.walletStore.walletlist[0]
    }
  }
  ...
  // _basicColorType(type){
  //   if(type == "basic"){
  //     return Color.basicColor;
  //   }
  //   if(type == "share"){
  ...
  // _gradientColorType(type){
  //   if(type == "basic"){
  //     return Color.basicGradient;
  //   }
  //   if(type == "share"){
  //     return Color.noticeGradient
  //   }
  // }
      ...
      return(
        <TouchableOpacity style={styles.mywalletitemlast} activeOpacity={1} 
          onPress={()=> 
            ...
            : 
            this.props.navigation.navigate("NewTokenAsset")}>
        ...
        <Animatable.View animation={"slideInRight"} duration={150 * (index + 1)} useNativeDriver>
          ...
          style={{marginBottom:10,borderRadius:5}}>
            <Ripple key={index} activeOpacity={0.9} style={[styles.mywalletitem]} 
              onPress={item.rvx_balance != "-" ? ()=> this.props.navigation.navigate("Transactions",{selectedWallet:item}) : null}>
              <FastImage source={{uri:item.LogoUrl}} style={styles.coinlogo} resizeMode={'contain'} />
              <View style={styles.coindetail}>
                <View style={styles.leftright}>
                  <Text style={[styles.mywalletitemname]}>{item.AssetCode.toUpperCase()}</Text>
                  <Text style={styles.totalcoin}>{item.rvx_balance ? `${item.rvx_balance}` : `0.00`}</Text>
                </View>
                <View style={[styles.leftright,{marginTop:3}]}>
                  <Text style={styles.wallettype}>{item.Name}</Text>
                  <Text style={styles.totalcurrency}>${item.CurrentPrice}</Text>
                </View>
              </View>
            </Ripple>
            ...
            if(walletlist.length > 0){
              ...
              walletlist.map(async(wallet,index) =>{
                ...
                web3.eth.call({
                  ...
                  this.setState({
                    refreshing:false,
                    mywalletlist: walletlist,//this.state.filtertype == "All" ? walletlist : walletlist.filter(x => x.wallettype == this.state.filtertype),
                    totalrvx: (isNaN(this.state.totalrvx) ? 0 : this.state.totalrvx) + balance
                  }, ()=>{
                    this.props.walletStore.setWallets(walletlist);
                    //await AsyncStorage.setItem('@wallet', JSON.stringify(this.state.mywalletlist));
                  });
                });
              });
            }else{
  ...
  // _animateInit(){
  //   if(this.state.selectedsnapindex  != this.state.mywalletlist.length || this.state.mywalletlist.length == 1 && this.state.selectedsnapindex  != this.state.mywalletlist.length){
  //     return "fadeInUp";
  //   }
  //   return "fadeOutDown";
  // }
  ...
  _onRefresh = () =>{
    this.setState({refreshing:true,totalrvx:0});
    this._loadWallet();
    this._loadCloudWallet();
  }
    ...
    const Gradient = ({ index }) => (
      <Defs key={index}>
          <LinearGradient id={'gradient'} x1={'0%'} y1={'0%'} x2={'0%'} y2={'100%'}>
              ...
              {/* <Stop offset={'15%'} stopColor={'rgba(100, 244, 244)'} stopOpacity={0.1}/> */}
              {/* <Stop offset={'50%'} stopColor={'rgb(100, 244, 244)'} stopOpacity={0.25}/>
              <Stop offset={'100%'} stopColor={'rgb(100, 244, 244)'} stopOpacity={0}/> */}
          </LinearGradient>
      </Defs>
    )
...
export default Home_backupbeforeerc;  
